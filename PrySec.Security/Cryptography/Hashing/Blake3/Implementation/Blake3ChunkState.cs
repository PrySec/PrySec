...
internal unsafe struct Blake3ChunkState
{
    ...
    public static void Update(Blake3ChunkState* self, byte* input, uint inputLength)
    {
        ...
        if (self->BufferLength > 0)
        {
            ...
            if (inputLength > 0)
            {
                CompressInPlaceImpl(self->Cv, self->Buffer, BLAKE3_BLOCK_LEN, self->ChunkCounter, self->Flags | MaybeStartFlag(self));
        ...
        {
            CompressInPlaceImpl(self->Cv, input, BLAKE3_BLOCK_LEN, self->ChunkCounter, self->Flags | MaybeStartFlag(self));
            self->BlocksCompressed++;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void ToOutput(Blake3ChunkState* self, Blake3Output* output)
    {
        Blake3Flags blockFlags = self->Flags | MaybeStartFlag(self) | Blake3Flags.CHUNK_END;
        Blake3Output.Make(output, self->Cv, self->Buffer, self->BufferLength, self->ChunkCounter, blockFlags);
    }
}
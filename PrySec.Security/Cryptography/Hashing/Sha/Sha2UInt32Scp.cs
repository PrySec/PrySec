...
public abstract unsafe class Sha2UInt32Scp : ShaUInt32Scp
{
    protected const int DIGEST_DWORD_LENGTH = 8;

    /// <summary>
    /// The length in bytes of the final Sha 256 digest
    /// </summary>
    protected const int DIGEST_LENGTH = DIGEST_DWORD_LENGTH * sizeof(uint);

    /// <summary>
    /// The length of the message schedule array in 32-bit words.
    /// </summary>
    protected const int MESSAGE_SCHEDULE_BUFFER_LENGTH = 64;

    protected const int BLOCK_SIZE = 16 * sizeof(uint);

    protected readonly uint[] H;
    ...
    protected Sha2UInt32Scp(uint[] initial)
    {
        H = initial;
    }

    private protected TOutputMemory HashCoreHelper<TOutputMemory>(ref ShaScpState state, uint* messageScheduleBuffer)
        where TOutputMemory : IUnmanaged<TOutputMemory, byte>
    {
        // create a buffer to hold the result
        TOutputMemory resultBuffer = TOutputMemory.Allocate(DIGEST_LENGTH);

        // get access to the possibly protected memory we just allocated
        using IMemoryAccess<uint> access = resultBuffer.GetAccess<uint>();

        // initialize result hash
        fixed (uint* pInitialHash = H)
        {
            Unsafe.CopyBlockUnaligned(access.Pointer, pInitialHash, DIGEST_LENGTH);
        }

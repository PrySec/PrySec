...
public abstract unsafe class Sha2UInt64Scp : ShaScpBase<ulong>
{
    ...
    private protected override void Initialize<T>(IUnmanaged<T> input, ref ShaScpState state)
    {
        ...
        *(UInt128BE_T*)(state.Buffer.BasePointer + state.Buffer.Count - 2) = new UInt128BE_T((ulong)state.DataLength << 3);

        // convert 64 bit word wise back to little endian.
        for (int i = 0; i < state.AllocatedSize; i++)
        {
            state.Buffer.BasePointer[i] = (UInt64BE_T)state.Buffer.BasePointer[i];
        }
    }
